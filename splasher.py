...
if __name__ == '__main__':
    ...
    if  wallpapers_Source == "random":
        ...
        path = path_c
        print("[INFO] Wallpaper SOURCE is {}".format(args["source"]))
        print("[INFO] Wallpaper COLLECTION ID is {}".format(args["c_id"]))
    # checking windows bit size

    system_type = struct.calcsize("P") * 8 # checking the windows bit size
    print("[INFO] Wallpaper INTERVAL is {}".format(args["interval"]))
    print("[INFO] Wallpaper RESOLUTION is {}".format(args["resolution"]))
    print("[INFO] Verbosity is {}".format(args["verbose"]))
    ...
    while True:
        ...
        if args["verbose"] == "ON" or args["verbose"] == "on":
            print(f"Wallpaper has been set and saved at location : {img_path}")
